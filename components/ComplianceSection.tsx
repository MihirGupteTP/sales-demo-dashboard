...
import { filterMeetings, formatDateTime, hubspotRecordUrl } from "@/lib/utils";
                ...
                {openSection === section.key && (
                  <div className="px-3 pb-3">
                    <div className="rounded-md border border-border bg-background overflow-hidden">
                      <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-2 px-3 py-1.5 border-b bg-muted/50 text-[11px] font-medium text-muted-foreground">
                        <span>Meeting</span>
                      ...
                      <div className="divide-y divide-border max-h-48 overflow-y-auto">
                        {section.issues.map((issue) => {
                          const linkUrl =
                            section.key === "no_deal"
                              ? hubspotRecordUrl("contact", issue.contactId)
                              : section.key === "no_lead"
                              ? hubspotRecordUrl("deal", issue.dealId) ?? hubspotRecordUrl("contact", issue.contactId)
                              : null;
                          return (
                            <div key={`${issue.meetingId}-${issue.type}`} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-2 px-3 py-2 text-xs">
                              ...
                              <span className="font-mono text-muted-foreground truncate">{issue.meetingId}</span>
